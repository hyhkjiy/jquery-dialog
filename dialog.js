...
;(function($, window, document, undefined){
	...
	Dialog.prototype = {
		open:function(){
			...
			,$dialog = $('<div></div>').appendTo($shade)		// 弹窗主体
			,$dialog_title = $('<div></div>').appendTo($dialog)		//弹窗标题
			,$dialog_tools = $('<div class="dialog_tools"></div>')	// 工具条
			,$close_btn = $('<a href="javascript:;">&times;</a>').appendTo($dialog_tools).on('click', function(){dialog.close()})	// 关闭按钮
			...
			// 自定义弹窗样式
			if(dialog.options.dialogStyle)$dialog.addClass(dialog.options.dialogStyle)
			...
			if(dialog.options.title)
				$dialog_title.addClass('dialog_title').html(dialog.options.title)	// 自定义标题
			else
				$dialog_title.detach()
			dialog.$dialog_shade = $shade.appendTo("body")	// 添加弹窗到文档
		...
		close:function(){
			...
			dialog.$dialog_shade.fadeOut(200, function(){	// 隐藏动画
				...
				if(dialog.$element.html)
					dialog.$element.html($(this).find('.dialog_body').html())	// 还原弹窗内容到元素内
